...
# --------------------------------------------------
# JsonLogic-backed document summary
# --------------------------------------------------
...
class DocumentSummaryModel(TransformBaseModel):
    ...
    passport_front: str | None = Field(
        default=None,
        transform=JSONLOGIC(
            {"var": "passport.passport_details.ovd_front.doc_name"}
        ),
    )
    ...
    passport_back: str | None = Field(
        default=None,
        transform=JSONLOGIC(
            {"var": "passport.passport_details.ovd_back.doc_name"}
        ),
    )
    ...
    instruction_doc: str | None = Field(
        default=None,
        transform=JSONLOGIC(
            {"var": "consultant_info.instruction_letter.upload_instruction.doc_name"}
        ),
    )
    ...
    application_form_doc: str | None = Field(
        default=None,
        transform=JSONLOGIC(
            {"var": "consultant_info.application_form.application_form.doc_name"}
        ),
    )
...
# --------------------------------------------------
# Location logic with branching + comparisons
# --------------------------------------------------

class LocationModel(TransformBaseModel):
    """Consolidated location information.

    Prefers residential address when available, otherwise falls
